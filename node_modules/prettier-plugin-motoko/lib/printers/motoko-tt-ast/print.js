...
exports.parseSpace = void 0;
const prettier_1 = require("prettier");
const spaceConfig_1 = __importStar(require("./spaceConfig"));
const utils_1 = require("./utils");
// Documentation: https://github.com/prettier/prettier/blob/main/commands.md
const { builders: { group, fill, ifBreak, breakParent, indent, indentIfBreak, 
// label,
line, softline, hardline, lineSuffix, hardlineWithoutBreakParent, literallineWithoutBreakParent, }, } = prettier_1.doc;
const space = ' ';
// const wrapIndent = indent(line);
const wrapIndent = line;
function parseSpace(input, a, b, leftMap, rightMap) {
    if (typeof input === 'string') {
        switch (input) {
            ...
            case 'keep-space':
                ...
                let result = space;
                do {
                    const token = (0, utils_1.getToken)(right);
                    if (token) {
                    ...
                    right && (right = rightMap.get(right));
                } while (right !== b);
                return result;
            default:
...
function shouldSkipTokenTree(tree) {
    const token = (0, utils_1.getToken)(tree);
    return !!token && removeTokenTypes.includes(token.token_type);
}
function getGroupDelimToken(groupType, hasWithKeyword) {
    return {
        token_type: 'Delim',
        data: groupType === 'Unenclosed' ||
            groupType === 'Curly' ||
            hasWithKeyword
            ? [';', 'Semi']
            : [',', 'Comma'],
...
function printTokenTree(tree, path, options, print, args) {
    if (tree.token_tree_type === 'Group') {
        ...
        const hasNestedGroup = (groupType === 'Square' || groupType === 'Paren') &&
            trees.length === 1 &&
        ...
        const isPossiblyRecordExtension = () => {
            ...
            let hasDelim = false;
            trees.forEach((tree) => {
                let token = (0, utils_1.getToken)(tree);
                if (!token) {
                    return false;
                }
        ...
        for (let i = 0; i < trees.length; i++) {
            ...
            if (a.token_tree_type === 'Token') {
                ...
                if (token.token_type === 'LineComment') {
                    comment = (0, utils_1.getTokenText)(token).slice(2).trim();
                }
                else if (token.token_type === 'BlockComment') {
                    comment = (0, utils_1.getTokenText)(token).slice(2, -2).trim();
                }
                else if (token.token_type === 'Ident' &&
                    token.data == 'with') {
                    hasWithKeyword = true;
                ...
                if (groupType === 'Comment') {
                    comment = (0, utils_1.getTokenTreeText)(a).slice(2, -2).trim();
                }
            }
            ...
            if (ignoringNextStatement) {
                ...
                let tt = a;
                do {
                    ignoreDoc.unshift((0, utils_1.getTokenTreeText)(tt));
                } while (leftMap.has(tt) &&
                ...
                if (!isDelim || i !== trees.length - 1) {
                    resultArray.push(isDelim
                        ? printToken(getGroupDelimToken(groupType, hasWithKeyword))
                        : printTokenTree(a, path, options, print, args));
                }
                if (i < trees.length - 1 && !isIgnoreComment) {
                    resultArray.push(printBetween(groupType, trees, i, i + 1, leftMap, rightMap));
                }
                    ...
                    if ((allowTrailingSeparator ||
                        ...
                        !(groupType === 'Square' &&
                            results.length === 1 &&
                            !isSeparator) // possibly array type
                    ) {
                        const delimDoc = printToken(getGroupDelimToken(groupType, hasWithKeyword));
                        results.push(
        ...
        endGroup();
        const pairSpace = results.length === 0
            ? []
        ...
        return shouldKeepSameLine() ? (0, utils_1.withoutLineBreaks)(resultDoc) : resultDoc;
    }
...
function printToken(token) {
    switch (token.token_type) {
        ...
        case 'MultiLine':
            return [breakParent];
        case 'LineComment':
            return ifBreak(token.data, `/* ${token.data.substring(2).trim()} */`);
    }
    return (0, utils_1.getTokenText)(token);
}
function printBetween(group, trees, aIndex, bIndex, leftMap, rightMap) {
    const rule = spaceConfig_1.default.rules.find(([aPattern, bPattern]) => {
        return ((0, spaceConfig_1.doesTokenTreeMatchPattern)(aPattern, trees, aIndex, group) &&
            (0, spaceConfig_1.doesTokenTreeMatchPattern)(bPattern, trees, bIndex, group));
    });
    return rule
