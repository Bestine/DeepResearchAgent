...
const tokenTypes = (types) => token((token) => types.includes(token.token_type));
const groupContains = (groupType, predicate) => (tt) => tt.token_tree_type === 'Group' &&
    tt.data[1] === groupType &&
    tt.data[0].some(predicate);
const and = (...conditions) => (tt) => conditions.every((condition) => condition(tt));
const or = (...conditions) => (tt) => conditions.some((condition) => condition(tt));
const not = (condition) => (tt) => !condition(tt);
// match both "block comments" and "comment groups" (lexer implementation detail)
const blockComment = (tt) => {
    var _a;
    return tt.token_tree_type === 'Group'
        ? tt.data[1] === 'Comment'
        : ((_a = (0, utils_1.getToken)(tt)) === null || _a === void 0 ? void 0 : _a.token_type) === 'BlockComment';
};
const spaceConfig = {
    ...
    rules: [
        ...
        [
            {
                left: and(tokenTypes(['Close', 'Ident', 'Literal']), not(keyword)),
