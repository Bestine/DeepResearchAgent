...
function preprocess(code, options) {
    ...
    if (options.semi) {
        ...
        code = reversedLines
            ...
            const nextLineMaskedCommentsTrimmed = (reversedLinesMaskedComments[nextLineIndex] || '').trim();
            // Original index of the next line
            const nextLineOriginalIndex = reversedLineIndices[nextLineIndex];
            if (trimmedLine === '}' &&
                // Skip if prefix found on next line
                (!nextLineMaskedCommentsTrimmed ||
                    skipAutomaticSemiForNextLinePrefixes.every((prefix) => !nextLineMaskedCommentsTrimmed.startsWith(prefix))) &&
                // Skip first block for if/else, try/catch
                !/^(else|catch|finally)([^a-zA-Z0-9_]|$)/.test(nextLineMaskedCommentsTrimmed) &&
                // Skip comments and string literals
                (nextLineOriginalIndex === undefined ||
                    !ignoreSpans.some(([start, end]) => start <= nextLineOriginalIndex &&
